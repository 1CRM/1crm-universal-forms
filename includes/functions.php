...
function ocrmf_process_request_vars() {
...
function ocrmf_map_meta_cap( $caps, $cap, $user_id, $args ) {
	$meta_caps = array(
		'ocrmf_edit' => 'publish_pages',
		'ocrmf_read' => 'edit_posts',
	);

	$meta_caps = apply_filters( 'ocrmf_map_meta_cap', $meta_caps );
...
function ocrmf_request_uri() {
	$uri = add_query_arg(array());
	return esc_url_raw( $uri );
}

function ocrmf_form($id) {
	$contact_form = new OneCRMForm($id);
